...
/// ONNX Runtime (through the VieNeuV2CodecONNX Objective-C++ wrapper — ORT's
...
/// 128-float vector for the selected preset voice, see VieNeuOfflineV2Voice)
/// into `vieneu_decoder_int8.onnx`.
actor VieNeuV2CodecDecoder {
    static let shared = VieNeuV2CodecDecoder()

    static let sampleRate = 24_000

    private var runner: VieNeuV2CodecONNX?
    ...
    private var voiceEmbeddings: [VieNeuOfflineV2Voice: [NSNumber]] = [:]

    private init() {}

    func decode(codes: [Int32], voice: VieNeuOfflineV2Voice) throws -> [Float] {
    ...
    private func loadedRunner() throws -> VieNeuV2CodecONNX {
        if let runner { return runner }
        guard let path = Bundle.main.path(forResource: "vieneu_decoder_int8", ofType: "onnx") else {
            throw VieNeuV2CodecError.modelMissing
        }
        let created = try VieNeuV2CodecONNX(modelPath: path)
        runner = created
        return created
    }

    private func loadedVoiceEmbedding(_ voice: VieNeuOfflineV2Voice) throws -> [NSNumber] {
        ...
        guard let path = Bundle.main.path(forResource: voice.rawValue, ofType: "json"),
            ...
            throw VieNeuV2CodecError.voiceEmbeddingMissing
        }
